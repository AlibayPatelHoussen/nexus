...
import { Search, RefreshCw, X, Tv2 } from 'lucide-react'
...
export default function AnimesPage() {
  ...
  const { data: recent, isLoading: loadRecent } = useQuery({
    queryKey: ['media', 'recent', 'anime'],
    queryFn:  () => mediaService.getRecent('anime', 16),
    enabled:  !hasFilters,
  })

  const { data: genres } = useQuery({
    queryKey: ['media', 'genres', 'anime'],
    queryFn:  () => mediaService.getGenres('anime'),
  })

  const { data: filtered, isLoading: loadFiltered } = useQuery({
    queryKey: ['media', 'list', 'anime', debouncedSearch, genre, page],
  ...
  async function scan() {
    setScanning(true)
    try {
      await mediaService.scan('animes')
  ...
  function clear() { setSearch(''); setGenre(''); setPage(1) }
      ...
      <div className="px-7 pb-10">
          ...
          <div>
            ...
            {loadFiltered ? (
              ...
              <div className="flex flex-col items-center justify-center h-40 gap-2">
                <Tv2 size={32} strokeWidth={1.5} style={{ color: 'var(--text3)' }} />
        ...
        {!hasFilters && !loadRecent && (!recent || recent.length === 0) && (
          <div className="flex flex-col items-center justify-center h-60 gap-3">
            <Tv2 size={48} strokeWidth={1} style={{ color: 'var(--text3)' }} />
            <p className="text-[14px] font-semibold" style={{ color: 'var(--text)' }}>
              Bibliothèque vide
            </p>
            <p className="text-[12px]" style={{ color: 'var(--text3)' }}>
              Placez vos animes dans /media/animes et lancez un scan
            </p>
